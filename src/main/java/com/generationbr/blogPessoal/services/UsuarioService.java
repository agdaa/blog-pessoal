	...
	public Optional<UsuarioLogin> logar (Optional<UsuarioLogin> usuarioParaLogar) {
		...
		if(usuario.isPresent()) {
			...
			if(encoder.matches(usuarioParaLogar.get().getSenha(), usuario.get().getSenha())) {
				...
				usuarioParaLogar.get().setId(usuario.get().getId());
				usuarioParaLogar.get().setFoto(usuario.get().getFoto());
				usuarioParaLogar.get().setTipoUsuario(usuario.get().getTipoUsuario());
